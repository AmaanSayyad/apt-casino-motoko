...
const SimpleBetting = ({ onSubmit, initialSettings = {} }) => {
  const [betAmount, setBetAmount] = React.useState(
    initialSettings.betAmount || 50
  );
  const [minesCount, setMinesCount] = React.useState(
    initialSettings.mines || 5
  );
    ...
    <div className="bg-gradient-to-br from-purple-900/20 to-blue-900/10 p-4 rounded-xl border border-purple-800/30">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="relative">
          <label className="block text-white/70 text-sm mb-1 font-sans">
            Bet Amount
          </label>
          <div className="flex items-center">
            ...
            <select
              ...
              onChange={(e) => {
                // Parse with Number instead of parseInt to preserve decimal values if needed
                const value = Number(e.target.value);
                console.log("Setting bet amount:", value);
                setBetAmount(value);
              }}
              value={betAmount}
            >
      ...
      <div className="mt-4 text-sm text-white/80 flex items-start bg-black/20 p-3 rounded-lg">
        ...
        <div>
          <span className="font-medium">How to play:</span> Set your bet amount
          and number of mines, then click tiles to reveal gems. Avoid bombs!
          Each safe tile increases your multiplier. Cash out anytime to secure
          your winnings.
        </div>
      </div>
    </div>
